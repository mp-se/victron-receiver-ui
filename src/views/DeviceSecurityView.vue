        ...
        <form @submit.prevent="saveSettings" class="needs-validation" novalidate>
            ...
            <div class="row" v-for="(device, index) in config.victron_config" :key="index">
                <div class="col-md-3 gy-2">
                    <input class="form-control" type="text" v-model="config.victron_config[index].name" maxlength="20" :disabled="global.disabled" :class="checkName(config.victron_config[index].name) ? '' : 'is-invalid'">
                </div>                            
                <div class="col-md-3 gy-2">
                    <input class="form-control" type="text" v-model="config.victron_config[index].mac" maxlength="17" placeholder="XX:XX:XX:XX:XX:XX" :disabled="global.disabled" :class="checkMac(config.victron_config[index].mac) ? '' : 'is-invalid'">
                </div>
                <div class="col-md-6 gy-2">
                    <input class="form-control" type="text" v-model="config.victron_config[index].key" maxlength="32" :disabled="global.disabled" :class="checkKey(config.victron_config[index].key) ? '' : 'is-invalid'">
                </div>
            </div>

            <div class="row">
                <div class="col-md-3">
                    <div class="form-text">Unique name of the device used to identify it in Home Assistant.</div>
                </div>
                <div class="col-md-3">
                    <div class="form-text">Paste the mac adress from the victron app, press save to validate and format the name.</div>
                </div>
                <div class="col-md-6">
                    <div class="form-text">Paste the encryption key from the Victron app, should be 32 chars.</div>
                </div>
            </div>
            ...
            <div class="row gy-2">
                ...
                <div class="col-sm-4">
                    <button @click="factory" type="button" class="btn btn-secondary" :disabled="global.disabled">
                        <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"
                            :hidden="!global.disabled"></span>
