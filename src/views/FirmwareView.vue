...
function upload() {
    ...
    if (fileElement.files.length === 0) {
        ...
        xhr.onloadend = function (e) {
            progress.value = 100
            if (xhr.status == 200) {
                global.messageSuccess = "File upload completed, waiting for device to restart before doing refresh!"
                global.messageFailed = ""
            }
            setTimeout(() => { location.href = location.href.replace('/other/firmware', '') }, 10000)
        }
