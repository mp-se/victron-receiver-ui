<template>
  <BsInputBase :width="width" :label="label" :help="help" :badge="badge">
    ...
    <input v-model="model" class="form-control" :type="type === undefined || flag ? 'text' : type" v-bind="$attrs" data-bs-toggle="tooltip"
      data-bs-custom-class="custom-tooltip" :data-bs-title="help">
      <span v-if="type === 'password'" class="input-group-text"><BsIcon @click="toggle" :icon="flag ? 'bi-eye-slash' : 'bi-eye'" width="1rem" height="1rem"/></span>
    </div>  
  </BsInputBase>
</template>
